...
function createPointerMoveHandler(
  ...
  return throttle((e: PointerEvent) => {
    ...
    // Stop propagation to prevent pointer event handlers in parent components or plugins
    // from interfering with the table-block's own hover and pointer logic. This helps avoid
    // conflicting state updates, unnecessary re-renders, or UI flicker, especially in environments
    // where multiple table-related plugins may be active and listening for pointer events.
    e.stopPropagation()
    ...
    const shouldEnterBoundary =
      distTableTop < enterThreshold ||
      distTableBottom < enterThreshold ||
      distTableLeft < enterThreshold ||
      distTableRight < enterThreshold ||
      distRowTop < enterThreshold ||
      distRowBottom < enterThreshold ||
      distColLeft < enterThreshold ||
      distColRight < enterThreshold
    ...
    const closeToBoundaryLeft =
      closeToBoundary &&
      (distTableLeft < exitThreshold || distColLeft < exitThreshold)
    const closeToBoundaryRight =
      closeToBoundary &&
      (distTableRight < exitThreshold || distColRight < exitThreshold)
    const closeToBoundaryTop =
      closeToBoundary &&
      (distTableTop < exitThreshold || distRowTop < exitThreshold)
    const closeToBoundaryBottom =
      closeToBoundary &&
      (distTableBottom < exitThreshold || distRowBottom < exitThreshold)
