...
function createPointerMoveHandler(
  ...
  return throttle((e: PointerEvent) => {
    ...
    if (closeToBoundary) {
      // Get the actual table element's boundary, not the wrapper div
      const tableElement = content.querySelector('table') || content
      const contentBoundary = tableElement.getBoundingClientRect()
      ...
      // For grid tables, allow adding rows anywhere (not just after header)
